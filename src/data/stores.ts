...
export const getStoreLogo = async (store: Store): Promise<string | null> => {
  ...
  const providers = [
    ...
    `https://logo.uplead.com/${baseName}.com`,
    `https://companieslogo.com/img/${baseName}-logo.png`,
    `https://companieslogo.com/img/${baseName}-logo-D-${baseName}.png`,
    
    // Brand repositories (reliable for major brands)
    `https://www.brandlogos.net/logos/${baseName}-logo.png`,
    `https://brandslogos.com/wp-content/uploads/${baseName}-logo.png`,
    `https://cdn.jsdelivr.net/npm/simple-icons@latest/icons/${baseName}.svg`,
    
    // Wikipedia variations (only try a couple)
    `https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/${baseName}-logo.svg/500px-${baseName}-logo.svg.png`,
    `https://upload.wikimedia.org/wikipedia/commons/thumb/b/b9/${baseName}-logo.svg/500px-${baseName}-logo.svg.png`,
  ].filter(Boolean) as string[];
