...
/* harmony export */ });
/* harmony import */ var _utils_supabase__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../utils/supabase */ "./src/utils/supabase.ts");
          ...
          `;
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    if (isLogin) {
                        const { data: { user }, error } = await _utils_supabase__WEBPACK_IMPORTED_MODULE_0__.supabase.auth.signInWithPassword({ email, password });
                        if (error)
                            throw error;
                        // Check if user profile exists
                        const { data: profile } = await _utils_supabase__WEBPACK_IMPORTED_MODULE_0__.supabase
                            .from('users')
                            .select('*')
                            .eq('id', user.id)
                            .single();
                        ...
                        if (!profile) {
                            const userId = await (0,_utils_user__WEBPACK_IMPORTED_MODULE_1__.getUniqueUserId)();
                            let name = user.email;
                            try {
                            ...
                            const { error: profileError } = await _utils_supabase__WEBPACK_IMPORTED_MODULE_0__.supabase
                                .from('users')
                                .insert({
                                id: user.id,
                                user_id: userId,
                        ...
                        const { data: { user }, error: signUpError } = await _utils_supabase__WEBPACK_IMPORTED_MODULE_0__.supabase.auth.signUp({ email, password });
...
async function isLoggedIn() {
    const { data: { session }, error } = await _utils_supabase__WEBPACK_IMPORTED_MODULE_0__.supabase.auth.getSession();
    if (error) {
...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   getUniqueUserId: () => (/* binding */ getUniqueUserId)
/* harmony export */ });
...
async function getUniqueUserId() {
    ...
    let exists = true;
    while (exists) {
        const { data } = await _supabase__WEBPACK_IMPORTED_MODULE_0__.supabase
            .from('users')
            .select('user_id')
            .eq('user_id', userId)
            .single();
...
/******/ 	(() => {
/******/ 		var inProgress = {};
/******/ 		var dataWebpackPrefix = "kindly-extension:";
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkkindly_extension"] = self["webpackChunkkindly_extension"] || [];
...
/* harmony import */ var _auth__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./auth */ "./src/content/auth.ts");
/* harmony import */ var _auth_form__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./auth-form */ "./src/content/auth-form.ts");
...
const getDonationPercentage = (merchant) => {
...
const isDomainActivated = (domain) => {
    const activations = getDomainActivations();
...
const showSuccessState = (merchant) => {
...
const createAffiliateChoiceDialog = (affiliateTag) => {
...
const createDonationPopup = async () => {
    ...
    const loggedIn = await (0,_auth__WEBPACK_IMPORTED_MODULE_1__.isLoggedIn)();
    if (!loggedIn) {
  ...
  `;
    ...
    <div style="padding: 24px;">
      <div style="text-align: center; margin-bottom: 24px;">
        ${loggedIn ? `
          ...
          <h3 style="color: #2D3648; font-size: 20px !important; font-weight: 600 !important; margin-bottom: 8px; line-height: 1.2 !important; letter-spacing: normal !important; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif !important;">
            Get up to to ${getDonationPercentage(currentMerchant)} donated to Susan G Komen
          </h3>
      ...
      ` : (0,_auth_form__WEBPACK_IMPORTED_MODULE_2__.createAuthForm)().html}
  ...
  `;
    ...
    if (loggedIn) {
        ...
        (0,_auth_form__WEBPACK_IMPORTED_MODULE_2__.createAuthForm)().setupListeners(container);
    }
};
